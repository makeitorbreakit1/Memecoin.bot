...
client.once(
  ...
  async (c) => {
    console.log(
      `[startup] Logged in as ${c.user.tag}`
    );
    ...
    watchlist =
      new Watchlist({
        ...
        requireRugCheck:
          REQUIRE_RUGCHECK.toLowerCase() ===
          "true",

        requireAuthorityData:
          REQUIRE_AUTHORITY_DATA.toLowerCase() ===
          "true",
    ...
    console.log(
      `[startup] Quality mode: score >= ${MIN_SCORE_TO_ALERT}, liquidity >= $${MIN_LIQUIDITY_USD}, verification >= ${MIN_VERIFICATION_CONFIDENCE_PCT}%, rug <= ${MAX_RUG_PROBABILITY_PCT}%`
    );
