...
class Watchlist {
  ...
  _extractRugCheckHolderData(
    ...
    /*
     * Full RugCheck reports normally expose:
     *
     * topHolders: [
     *   {
     *     owner: "...",
     ...
     * ]
     *
     * Support several field-name variations.
     */
    ...
    const raw =
      Array.isArray(
        ...
        ? rugCheckReport.topHolders
        : Array.isArray(
            rugCheckReport?.data?.topHolders
          )
          ? rugCheckReport.data.topHolders
          : [];
  ...
  async _attachHolderData(
     ...
     * ---------------------------------------------------------------
     *
     * Use the FREE Solscan Token Meta endpoint.
     *
     * This is display-only and does not block an otherwise
     * qualified token.
     */
    try {
      const solscanMeta =
  ...
  _hardSafetyBlock(
     ...
     * TOP HOLDER SAFETY
     *
     * RugCheck is the source for this.
     */
    const topHolder =
      Number(
        snapshot.topHolderPct
    ...
    /*
     * Additional top-10 concentration filter.
     */
    const top10 =
      Number(
        snapshot.top10HolderPct
  ...
  async pollOnce() {
    ...
    /*
     * SEND ONLY THE BEST TOKEN THIS CYCLE
     */
    const winners =
      qualified.slice(
        0,
    ...
    for (
      ...
      console.log(
        `[poll] Top 20 wallets: ${
          snapshot.top20HolderPct !=
          null
            ? snapshot.top20HolderPct.toFixed(
                1
              ) + "%"
            : "N/A"
        }`
    ...
    if (
      ...
      console.log(
        `[poll] ${suppressed} additional qualified token(s) suppressed.`
      );
    }
