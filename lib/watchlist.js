...
class Watchlist {
  ...
  async _assessRug(address, snapshot, liqMcPct) {
    // RugCheck and Solana mint/freeze authority checks are Solana-specific.
    // For other chains, return a transparent "unavailable" assessment rather
    // than routing the token through the Solana-only safety pipeline.
    if (snapshot?.chain !== "solana") {
      return {
        rugProbabilityPct: null,
        riskLevel: "UNAVAILABLE",
        reasons: ["Chain-specific contract/rug checks are not available in V6 for this network"],
        mintAuthorityRenounced: null,
        freezeAuthorityRenounced: null,
        rugCheckAvailable: false,
        chainSpecificChecksAvailable: false,
      };
    }
    ...
    return { ...assessRugRisk({ snapshot, liqMcPct, mintAuthorityStatus, rugCheckReport }), chainSpecificChecksAvailable: true };
  }

  _finalVerificationConfidence(snapshot, rugAssessment) {
    let confidence = 0;
    if (snapshot?.chain !== "solana") {
      if (snapshot?.dexPair && snapshot?.liquidityUsd != null && snapshot?.marketCapUsd != null && snapshot?.volume24hUsd != null) confidence += 70;
      if (snapshot?.buySellRatio24 != null) confidence += 15;
      if (snapshot?.priceUsd != null) confidence += 15;
  ...
  async evaluateOne(tokenAddress) {
    try {
      ...
      // Non-Solana radar is market-data mode: don't demand Solana-only
      // authority/RugCheck data that cannot exist on EVM chains.
      if (snapshot.chain !== "solana") {
        return { snapshot, result, rugAssessment, holderIntelligence: { available: false, reason: "Solana-only holder intelligence skipped" }, behavior: compareHolderSnapshots(null, null), marketBehavior: classifyMarketBehavior({ snapshot, holderIntel: null, behavior: null }) };
      }
