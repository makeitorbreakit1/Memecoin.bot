...
class Watchlist {
  ...
  async pollOnce() {
    ...
    for (const address of candidates) {
      ...
      if (snapshot.verificationConfidencePct < 80) continue;
      if (!snapshot.authorityDataAvailable) continue;
      if (!rugAssessment.rugCheckAvailable && rugAssessment.rugProbabilityPct == null) continue;
      ...
      try { await this.onAlert(snapshot, result, rugAssessment, sniperSignal); }
