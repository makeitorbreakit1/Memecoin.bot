...
class Watchlist {
  ...
  async _attachHolderData(
    ...
    try {
      ...
      if (
        ...
        console.log(
          `[holders] ${address}: Solscan holder data unavailable`
        );
      ...
      console.log(
        `[holders] ${address}: ${
          holderData.holderCount != null
            ? holderData.holderCount.toLocaleString()
            : "N/A"
        } holders`
      );
      ...
      console.warn(
        `[holders] Solscan lookup failed for ${address}: ${err.message}`
      );
  ...
  async _getHolderBehavior(
    ...
    return {
      holderIntelligence: {
        ...
        reason:
          "Detailed holder behavior history is disabled; Solscan holder distribution is used for current holder concentration.",
      },
  ...
  async evaluateOne(
    ...
    try {
      ...
      /*
       * Holder data is fetched only for manual / slash-command checks.
       */
  ...
  async pollOnce() {
    ...
    for (
      ...
      console.log(
        `[poll] ${address} verification confidence: ${verificationConfidencePct}%`
      );
      ...
      if (
        ...
        console.log(
          `[poll] ${address} rejected: verification confidence ${verificationConfidencePct}% < ${this.minVerificationConfidencePct}%`
        );
       ...
       * - verification
       *
       * This prevents the holder API from being hammered by all 30-40
      ...
      if (
        ...
        console.log(
          `[poll] ${address} rejected: holder data unavailable`
        );
      ...
      console.log(
        `[poll] ${address} QUALIFIED: score ${result.score}, rug ${rugAssessment.rugProbabilityPct}%, verification ${verificationConfidencePct}%, holders ${
          snapshot.holderCount !=
          null
    ...
    /*
     * -------------------------------------------------------------------------
     * PICK ONLY THE BEST TOKEN FROM THE CYCLE
     * -------------------------------------------------------------------------
     *
     * This is what prevents the Discord channel from being flooded by several
     * tokens that all barely qualify.
     */
    if (
      qualified.length ===
      0
    ...
    qualified.sort(
      (a, b) => {
        ...
        const bHolders =
          Number(
            b.snapshot.holderCount
          ) || 0;

        const aHolders =
          Number(
            a.snapshot.holderCount
          ) || 0;
    ...
    if (
      ...
      console.log(
        `[poll] ${skipped} additional qualified token(s) were suppressed to prevent Discord flooding.`
      );
    }
