...
class Watchlist {
  ...
  _extractHolderCount(rugCheckReport) {
    if (!rugCheckReport) {
      return null;
    }

    const candidates = [
      rugCheckReport.totalHolders,
      rugCheckReport.holders,
      rugCheckReport.holderCount,
      rugCheckReport.token?.totalHolders,
      rugCheckReport.token?.holderCount,
      rugCheckReport.tokenMeta?.totalHolders,
      rugCheckReport.tokenMeta?.holderCount,
      rugCheckReport.summary?.totalHolders,
      rugCheckReport.summary?.holderCount,
      rugCheckReport.data?.totalHolders,
      rugCheckReport.data?.holderCount,
    ];
    ...
    /*
     * Some RugCheck responses expose the holder accounts
     * directly rather than a precomputed total.
     */
    const holderArrays = [
      rugCheckReport.topHolders,
      rugCheckReport.holders,
      rugCheckReport.token?.topHolders,
      rugCheckReport.token?.holders,
      rugCheckReport.data?.holders,
    ];
  ...
  async _assessRug(
     ...
     * -------------------------------------------------------------------------
     *
     * Pull the total holder count from RugCheck instead of using
     * getTokenLargestAccounts from Solana RPC.
     *
     * This avoids the RPC 429 problem you were seeing.
     */
    const holderCount =
      this._extractHolderCount(
        rugCheckReport
      );
    ...
    return {
      ...
      /*
       * Keep the raw RugCheck report available internally.
       */
      rugCheckReport,
