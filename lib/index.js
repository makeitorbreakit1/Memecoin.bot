...
for (const key of ["DISCORD_TOKEN", "CHANNEL_ID", "HELIUS_API_KEY"]) {
  if (!process.env[key]) {
...
async function registerCommands() {
  ...
  try {
    ...
    console.log("[startup] Slash command /radar registered for Solana.");
  } catch (err) {
...
async function runRadar(address, reply) {
  ...
  if (!evaluation) {
    return reply("Couldn't find a usable Solana DEX pair for that mint address.");
  }
...
client.once(Events.ClientReady, async (c) => {
  ...
  watchlist = new Watchlist({
    ...
    rpcUrl: RPC_URL || `https://mainnet.helius-rpc.com/?api-key=${HELIUS_API_KEY}`,
  ...
  console.log(`[startup] Solana polling every ${POLL_INTERVAL_MS}ms.`);
