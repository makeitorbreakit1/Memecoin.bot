...
function assessRugRisk({ snapshot, liqMcPct, mintAuthorityStatus, rugCheckReport }) {
  ...
  const mintActive = mintAuthorityStatus?.available ? mintAuthorityStatus.mintAuthority != null : snapshot.mintAuthority != null;
  const freezeActive = mintAuthorityStatus?.available ? mintAuthorityStatus.freezeAuthority != null : snapshot.freezeAuthority != null;

  if (mintActive) { risk += 24; reasons.push("Mint authority is active"); }
  if (freezeActive) { risk += 22; reasons.push("Freeze authority is active"); }
  ...
  return {
    ...
    mintAuthorityRenounced: !mintActive,
    freezeAuthorityRenounced: !freezeActive,
