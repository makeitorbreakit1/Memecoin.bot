...
function scoreToken(s) {
  ...
  const authorityKnown = s.authorityDataAvailable === true;
  const mintSafe = authorityKnown && s.mintAuthority == null;
  const freezeSafe = authorityKnown && s.freezeAuthority == null;
  if (authorityKnown) {
    add("Mint authority", mintSafe ? 5 : 0, 5, mintSafe ? "renounced" : "active");
    add("Freeze authority", freezeSafe ? 5 : 0, 5, freezeSafe ? "renounced" : "active");
  }
