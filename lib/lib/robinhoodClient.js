...
async function discoverRobinhoodLaunches() {
  // Official launchpad.meme partner docs expose this as the canonical new-launch feed.
  const urls = [
    `${LAUNCHPAD_BASE}/token-list/robinhood.json`,
    `${LAUNCHPAD_BASE}/api/public/tokens/new?chain=robinhood`,
  ];

  for (const url of urls) {
    try {
      const body = await requestJson(url, { headers: { Accept: "application/json" } }, 12000, 1);
      const normalized = arrFrom(body).map(normalizeLaunch).filter(Boolean);
      if (normalized.length) return normalized.slice(0, 100);
    } catch (err) {
      console.warn(`[robinhoodClient] Launch feed failed: ${err.message}`);
    }
  }
  return [];
}
