...
async function discoverRobinhoodLaunches() {
  ...
  // Fetch token profiles & boosts from DexScreener (Free & Reliable)
...
async function getRobinhoodTokenData(launch, opts = {}) {
  ...
  const contract = await getContractChecks(address, opts.rpcUrl || RH_RPC_DEFAULT);

  const totalSupplyRaw = contract?.totalSupplyRaw || null;
  const now = Date.now();
  ...
  return {
    ...
    name: launch.name || pair?.baseToken?.name || "Unknown",
    symbol: launch.symbol || pair?.baseToken?.symbol || "TOKEN",
