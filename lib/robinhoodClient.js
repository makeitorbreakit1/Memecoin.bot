...
const BLOCKSCOUT_BASE = process.env.BLOCKSCOUT_BASE || "https://api.blockscout.com/4663/api/v2";
...
async function discoverRobinhoodLaunches() {
  ...
  // 1. Fetch from Blockscout API Gateway Indexer
  try {
    const blockscoutData = await requestJson(`${BLOCKSCOUT_BASE}/tokens?type=ERC-20`, {}, 10000, 1);
    const items = arrFrom(blockscoutData);
    ...
    console.warn(`[robinhoodClient] Blockscout token discovery skipped: ${err.message}`);
  }

  // 2. Fallback / supplement with DexScreener profiles & boosts
  const dexUrls = [`${DEX_BASE}/token-profiles/latest/v1`, `${DEX_BASE}/token-boosts/latest/v1`];
  for (const url of dexUrls) {
    try {
      const body = await requestJson(url, {}, 10000, 1);
