...
async function discoverRobinhoodLaunches() {
  ...
  // 1. Fetch from Blockscout Token Indexer for complete network coverage
  try {
    const blockscoutData = await requestJson(`${BLOCKSCOUT_BASE}/tokens?type=ERC-20`, { headers: { Accept: "application/json" } }, 10000, 1);
    ...
    console.warn(`[robinhoodClient] Blockscout token discovery failed: ${err.message}`);
  }
