...
function scheduleHelius(task) {
  const run = rpcQueue.then(async () => {
    ...
    while (true) {
      try {
        ...
        // ---------------------------------------------------------------------
        // Fail instead of repeatedly hammering Helius.
        // ---------------------------------------------------------------------
        ...
        if (
          ...
          console.warn(
            `[apiClient] Helius RPC unavailable after ` +
              `${HELIUS_MAX_RETRIES} retry. ` +
              `Skipping on-chain check for this token.`
          );
  ...
  // Keep queue alive even if something unexpected escapes.
  rpcQueue =
    run.catch(() => null);
...
async function rpcCall(
  ...
  const cacheKey =
    `rpc:${rpcUrl}:${method}:${JSON.stringify(
      params
    )}`;
...
async function getRugCheckReport(
  ...
  for (const suffix of [
    ...
    try {
      ...
      // -----------------------------------------------------------------------
      // Try unauthenticated RugCheck request if API key is rejected.
      // -----------------------------------------------------------------------
