...
const solscanHolderCache = new Map();

// -----------------------------------------------------------------------------
// SOLSCAN SETTINGS
// -----------------------------------------------------------------------------

const SOLSCAN_CACHE_MS = Math.max(
  60000,
  Number(
...
// -----------------------------------------------------------------------------
//
// Solscan v2 token/holders returns:
//   data.total  -> total holders
//   data.items  -> ranked holders
//
// Each holder record includes owner + percentage.
// We aggregate by owner so one wallet does not get counted twice.
// -----------------------------------------------------------------------------

async function getSolscanHolderData(
  address,
  apiKey
) {
  if (
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `[apiClient] Solscan returned no holder items for ${address}.`
      );
    ...
    console.log(
      `[holders] Solscan ${address}: ${
        output.holderCount != null
          ? output.holderCount.toLocaleString()
          : "N/A"
      } holders, top holder ${
        output.topHolderPct != null
          ? output.topHolderPct.toFixed(
              1
            ) + "%"
          : "N/A"
      }, top 10 ${
        output.top10Pct != null
          ? output.top10Pct.toFixed(
              1
            ) + "%"
          : "N/A"
      }`
    ...
    if (
      ...
      if (
        ...
        console.warn(
          `[apiClient] Solscan holder API rate limited.`
        );
      ...
      console.warn(
        `[apiClient] Solscan holder lookup failed for ${address}: ${err.message}`
      );
    }
