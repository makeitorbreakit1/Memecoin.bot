...
// Space Helius requests apart.
const HELIUS_RPC_INTERVAL_MS = Math.max(
  1000,
  Number(process.env.HELIUS_RPC_INTERVAL_MS || 1000)
);

// Helius cooldown after 429.
// We no longer sit through repeated retries.
...
// IMPORTANT:
// Default is now 0.
// If Helius says 429, immediately use fallback RPC.
...
function scheduleHelius(task) {
  const run =
    rpcQueue.then(
      async () => {
        // ---------------------------------------------------------------------
        // If Helius is cooling down, wait only when another Helius request is
        ...
        while (true) {
          try {
            ...
            // Default = no retry.
            //
            // We immediately return null so rpcCall() can use the fallback RPC.
            // -----------------------------------------------------------------
            ...
            // -----------------------------------------------------------------
            // Optional retry if user explicitly sets MAX_RETRIES > 0.
            // -----------------------------------------------------------------
...
async function directRpcCall(
  ...
  const doRequest =
    async () => {
      ...
      // -----------------------------------------------------------------------
      // Solana JSON-RPC error inside HTTP 200.
      // -----------------------------------------------------------------------
...
async function getRugCheckReport(
  ...
  for (
    ...
    try {
      ...
      // -----------------------------------------------------------------------
      // Try unauthenticated request if API key rejected.
      // -----------------------------------------------------------------------
