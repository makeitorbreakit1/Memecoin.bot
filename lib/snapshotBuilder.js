...
async function buildSnapshot(
  ...
  /*
   * We only need getAccountInfo here.
   *
   * getTokenSupply is redundant because the parsed Mint account contains
   * supply + decimals.
   *
   * getTokenLargestAccounts is intentionally removed from the normal radar
   * flow because it was repeatedly hitting RPC 429 errors.
   */
  if (rpcUrl) {
    mintAccount =
      await rpcCall(
  ...
  /*
   * Mint account data contains supply and decimals.
   */
  const decimals =
    num(parsedMint.decimals) ??
    num(pair.baseToken?.decimals);

  const totalSupplyRaw =
    num(parsedMint.supply);
  ...
  return {
    ...
    /*
     * Top holder percentage is intentionally null until we have a reliable
     * holder-distribution source. We do NOT pretend the holder count is the
