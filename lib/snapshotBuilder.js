...
async function buildSnapshot(address, opts = {}) {
  ...
  /*
   * Core market + blockchain information.
   *
   * Birdeye is OPTIONAL.
   */
  const [
  ...
  const pair = bestPair(
    pairs.length ? pairs : dexTokenData
  );

  if (!pair) {
    throw new Error("No Solana DEX pair found");
  }

  /*
   * Birdeye remains optional.
   *
   * If BIRDEYE_API_KEY is removed from Railway,
  ...
  const decimals =
    num(supplyValue?.decimals) ??
    num(heliusAsset?.token_info?.decimals) ??
    num(overview?.decimals);
  ...
  /*
   * Helius is now the primary holder source.
   *
   * Birdeye is only a fallback if it happens to be available.
   */
  const holderCount =
    num(heliusHolderCount) ??
    num(overview?.holder) ??
    num(overview?.holders) ??
    num(overview?.holderCount) ??
    null;
  ...
  return {
    ...
    name:
      pair.baseToken?.name ??
      overview?.name ??
      heliusAsset?.content?.metadata?.name ??
      "Unknown Token",
    ...
    priceChange6hPct:
      metrics.priceChange6hPct,

    priceChange24hPct:
      metrics.priceChange24hPct,
    ...
    liquidityLockedPct:
      num(security?.lpLockedPct) ??
      num(
        security?.lpLockedPercentage
      ),
    ...
    /*
     * These are optional supplemental sources.
     */
    birdeyeOverview:
      overview,
