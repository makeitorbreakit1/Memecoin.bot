...
async function buildSnapshot(address, opts = {}) {
  ...
  const pairs =
    await getDexTokenPairs(address);
  ...
  if (rpcUrl) {
     ...
     * IMPORTANT:
     *
     * These calls are deliberately sequential.
     *
     * apiClient.js serializes Helius requests and handles rate limits.
     *
     * If Helius temporarily fails, rpcCall() returns null.
     *
     * We do NOT pretend that missing authority data means the token
     * is safe. The Watchlist safety layer will fail closed and skip
     * the token when authority verification is unavailable.
     */

    supply = await rpcCall(
      rpcUrl,
      "getTokenSupply",
      [address]
    );

    largest = await rpcCall(
      rpcUrl,
      "getTokenLargestAccounts",
      [address]
    );

    mintAccount = await rpcCall(
      rpcUrl,
      "getAccountInfo",
      [
        address,
  ...
  const mintAuthority =
    parsedMint.mintAuthority ?? null;

  const freezeAuthority =
    parsedMint.freezeAuthority ?? null;

  // Detailed holder enumeration intentionally disabled.
