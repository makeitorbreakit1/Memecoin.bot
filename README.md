...
The folder is already a git repo with an initial commit (see below if you
need to redo it). To push to a new GitHub repository:
...
If you're starting fresh instead of using the included commit:

```bash
git init
git add .
git commit -m "Initial commit: memecoin radar bot"
git remote add origin https://github.com/<your-username>/<your-repo>.git
git branch -M main
git push -u origin main
```

`.env` is already git-ignored, so your token and API keys won't be
committed — only `.env.example` is tracked. There's no `package-lock.json`
committed (it couldn't be generated in the environment this was built in);
run `npm install` locally once and commit the generated lockfile if you want
fully reproducible installs, or just let Railway/Nixpacks generate it during
the build.

## Deploying to Railway

This repo includes `railway.json` (Nixpacks builder, `npm start` as the
start command, restart-on-failure) and a `Procfile` as a fallback, plus
...
4. Go to your service's **Variables** tab and add everything from `.env.example`:
   `DISCORD_TOKEN`, `CHANNEL_ID`, and optionally `BIRDEYE_API_KEY`,
   `HELIUS_API_KEY`, `PING_ROLE_ID`, `POLL_INTERVAL_MS`,
   `MIN_TOKEN_AGE_SECONDS`, `MAX_TOKEN_AGE_SECONDS`, `MIN_SCORE_TO_ALERT`.
5. Deploy. Check the **Deploy Logs** for `[startup] Logged in as ...` to
   confirm the bot connected.

**Important:** this is a background worker, not a web server — it never
binds to a port. If Railway's dashboard shows a "no healthcheck" notice or
asks about a public domain, ignore it / don't generate a domain; the service
type here is a worker, and `railway.json`'s config reflects that (no
`healthcheckPath` is set, so Railway won't wait on an HTTP response before
considering the deploy healthy).

Since the bot keeps in-memory dedupe state (`alertedMints` in
`watchlist.js`), a Railway redeploy or restart will reset it and the bot may
re-alert on tokens it already flagged. That's expected with the current
storage approach — see the note at the bottom of this file if you want to
persist it.
